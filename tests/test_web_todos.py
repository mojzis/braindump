...
@pytest.fixture
def priority_todos(monkeypatch, cfg):
    _set_home(monkeypatch, cfg)
...
@pytest.mark.anyio
async def test_todos_priority_filter(priority_todos):
...
@pytest.mark.anyio
async def test_todos_priority_sort(priority_todos):
    ...
    assert f"/entries/{priority_todos.entry.id}" in sorted_rows.text


@pytest.mark.anyio
async def test_todos_blank_priority_filter_keeps_all_priorities(priority_todos):
...
async def test_todos_descending_priority_keeps_noncanonical_values_last(
    ...
    body = response.text
    assert response.status_code == 200
    assert body.index(f"priority {earlier}") < body.index(f"priority {later}")
