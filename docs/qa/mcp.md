# Todo presence and partial body edits over real stdio
...
The maintained consumer also covers todo presence create/show/update/clear and
exact, unclassified, and Needs my time filtering.
...
proves legacy whole-body and metadata updates, todo presence create/show/update/
clear, invalid-value rejection, the `update` tool schema, and a
...
Presence pass conditions: `create` stores `agent` and `together`, `show` returns
the values, `update` changes only presence while body/input remain unchanged,
`clear_presence` removes the field, invalid values and non-todo use fail, and
`search`/`list` return the exact, unclassified, and together+personal union.
